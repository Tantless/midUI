...
export const AiRightClickNode = memo(({ id, data, selected }: Props) => {
  ...
  return (
    ...
    >
      <div className="flex items-center gap-2 mb-2">
        ...
        <span className="text-sm font-semibold text-slate-700">右键</span>
      </div>
      <input
        type="text"
        value={data.params.target || ''}
        placeholder="元素选择器或描述"
        className="w-full px-2 py-1 text-sm border border-slate-300 rounded focus:outline-none focus:border-blue-600"
...
AiRightClickNode.displayName = 'AiRightClickNode';
